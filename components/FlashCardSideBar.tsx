...
const FlashCardSideBar = ({ chats, chatId }: Props) => {
  return (
    <div className="w-full max-h-screen overflow-scroll soff p-4 text-blue-800 bg-blue-50">
      <Link href="/cognohub">
        <Button size="lg" variant="secondary" className="w-full bg-blue-600 text-white hover:bg-blue-700">
      ...
      <div className="flex max-h-screen overflow-scroll pb-20 flex-col gap-2 mt-4">
        {chats.map((chat) => (
          <Link key={chat.id} href={`/flashCard/${chat.id}`}>
            <div
