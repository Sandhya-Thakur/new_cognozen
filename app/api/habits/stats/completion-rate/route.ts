...
export async function GET(req: Request) {
  try {
    ...
    const processedResults = results.map(result => {
      ...
      const completionRate = (result.actualCompletions / adjustedExpectedCompletions) * 100;

      return {
        habitId: result.habitId,
        habitName: result.habitName,
        rate: Math.min(completionRate, 100), // Cap at 100%
      };
    });

    return NextResponse.json({ data: processedResults });
  } catch (error) {
    console.error("Error fetching habit completion statistics:", error);
    return NextResponse.json(
      { error: "An error occurred while fetching habit completion statistics" },
      { status: 500 }
    );
  }
}