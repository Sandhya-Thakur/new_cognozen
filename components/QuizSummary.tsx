...
const QuizSummary: React.FC = () => {
  ...
  const [displayedQuizzes, setDisplayedQuizzes] = useState<QuizSummary[]>([]);
  ...
  useEffect(() => {
    if (data) {
      ...
      setDisplayedQuizzes(sortedQuizzes.slice(0, 6));
    }
  }, [data]);
  ...
  if (isError) {
    ...
    if (axios.isAxiosError(error)) {
      if (error.response?.status === 401) {
        return <div className="text-center text-[#0F52BA] p-4">Please log in to view your quiz summary.</div>;
      }
    }
    return <div className="text-center text-[#0F52BA] p-4">Error loading quiz summary: {error.message}</div>;
  }
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {displayedQuizzes.length > 0 ? (
          displayedQuizzes.map((quiz) => (
            <Card key={quiz.id} className="overflow-hidden border border-[#0F52BA] bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-sm font-medium text-[#0D47A1]">{quiz.progress}%</span>
                    </div>
                  ...
                  {quiz.score !== null && (
                    <div className="flex items-center">
                      ...
                      <span className="text-sm font-medium text-[#0D47A1]">Score: {quiz.score}%</span>
                    </div>
                  )}
                  <Accordion
                    type="single"
          ...
          <div className="col-span-full text-center text-[#0D47A1] p-4">No quizzes found. Start a new quiz to see your summary here!</div>
        )}
      </div>
    </div>
  );
